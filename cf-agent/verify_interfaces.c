...
PromiseResult VerifyInterfacePromise(EvalContext *ctx, const Promise *pp)
{
    ...
    TryLLDPDiscovery(ctx, pp->promiser);
...
static void TryLLDPDiscovery(EvalContext *ctx, char *interface)
{
    ...
    snprintf(cmd, CF_BUFSIZE, "%s show neighbors %s -f keyvalue", CF_LLDP_TOOL, interface);
    ...
    snprintf(cmp, CF_MAXVARSIZE, "lldp.%s.", interface);
    ...
    while (!feof(pfp))
    {
        ...
        */
        ...
        if (strncmp("chassis.name=", offset, strlen("chassis.name=")) == 0)
        {
            ...
            EvalContextVariablePutSpecial(ctx, SPECIAL_SCOPE_SYS, varname, VDOMAIN, CF_DATA_TYPE_STRING, "inventory, source=lldp");
        }
